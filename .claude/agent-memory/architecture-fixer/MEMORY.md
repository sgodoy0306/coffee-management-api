...
- [immutable-collection-hibernate-merge.md](immutable-collection-hibernate-merge.md) — R61: `.toList()` (Java 16+) returns immutable list; Hibernate merge calls `.clear()` on it → UnsupportedOperationException. Always use `ArrayList` for JPA-managed collections.
